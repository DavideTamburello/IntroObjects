...
internal class Program
{
    private static void Main(string[] args)
    {
        ...
        //stampa scatole con l'ordinamento aggiuntivo - per area di base
        Console.WriteLine("stampa delle scatole ordinate per area di base - uso del comparatore CompareByBaseArea");
        magazzino.Boxes.Sort(new CompareByBaseArea());

        //test di ordinamento inverso
        Console.WriteLine("stampa delle scatole ordinate per volume in ordine inverso");
        magazzino.Boxes.Sort(new ComparerInverso());
        magazzino.StampaScatole();

        //per ordinare in ordine inverso si può anche fare l'ordinamento naturale e successivamente richiamare il Reverse()
        magazzino.Boxes.Sort(); //ordinamento naturale della lista
        magazzino.Boxes.Reverse(); //sposta in ordine inverso gli elementi
        magazzino.StampaScatole();

        //test sulle uguaglianze aggiuntive
        Box box1 = new Box(1, 2, 3);
        Box box2 = new Box(3, 2, 1);
        //test su uguaglianza predefinita (quella definita in IEquatable<Box> )
        if(box1.Equals(box2)) //box1 == box2
        {
            Console.WriteLine("box1 e box2 sono uguali");
        }else
        {
            Console.WriteLine("box1 e box2 sono diverse");
        }
        //test su SommaDimensioniEqualityComparer
        SommaDimensioniEqualityComparer comparer = new SommaDimensioniEqualityComparer();
        Console.WriteLine("test di SommaDimensioniEqualityComparer");
        if(comparer.Equals(box1, box2))
        {
            Console.WriteLine("box1 e box2 sono uguali in base al criterio SommaDimensioniEqualityComparer");
